...
def _pkl_codegen_impl(ctx):
    ...
    # Sandboxed builds don't allow writes to pkl's default cache location.
    ...
    ctx.actions.run(
        ...
        outputs = [outdir],
        executable = ctx.executable._pkl_codegen_cli,
        arguments = [gen_args],
...
def pkl_config_java_library(name, files, module_path = [], generate_getters = None, deps = [], tags = [], **kwargs):
    ...
    pkl_deps = [artifact("com.apple.pkl:pkl-config-java")]
