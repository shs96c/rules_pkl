load(":utils.bzl", "bazel_repo_name", "generate_lock_hash", "parse_pkl_dep")
...
def _read_deps_from_lock_file(rctx):
    ...
    if expected_hash != actual_hash:
        ...
        if not "REPIN" in env_vars and not "REPIN_PKL" in env_vars:
            fail(message)
        print(message)
...
def load_pkl_hub_deps():
"""

    for module in modules:
        parsed = parse_pkl_dep(module["name"])

        repo = repos.get(parsed.repo)

        # Figure out the URL
        url = "{repo}{module_name}/{version}/{simple_name}-{version}.pkl".format(
            repo = repo,
...
def _generate_build_file(name, raw_deps, repos, lock_file_path, modules):
    build_file_content = """
load("@apple_federation//pkl/private:pkl_library.bzl", "pkl_library")
load("@apple_federation//pkl/private:pkl_cache_entry.bzl", "pkl_cache_entry")
load("@apple_federation//pkl/private:pkl_pin.bzl", "pkl_pin")
...
pkl_hub_deps = repository_rule(
    _pkl_hub_deps_impl,
    doc = """Used for caching items from Pkl Hub so they don't need to be downloaded by the Pkl binary.""",
    attrs = {
        "deps": attr.string_list(
            allow_empty = True,
            doc = "A list of Pkl \"module URI\"s.",
        ),
        "repositories": attr.string_dict(
            default = {
                "applehub": "https://artifacts.apple.com/pkl/",
            },
            doc = "A dict mapping Pkl Hub name (eg. `applehub`) to a base URL.",
        ),
